...
async function processImage(metaImage: typeof metaImages.$inferSelect) {
  console.log(`Processing image ID: ${metaImage.id}, URL: ${metaImage.image_url}`);

  if (!metaImage.image_url || !metaImage.image_url.startsWith(STATIC_DOMAIN_PREFIX)) {
    console.log(`Skipping image ID: ${metaImage.id} due to invalid or non-static URL.`);
    return;
  }

  const originalImageUrl = metaImage.image_url;
  const transformOptions = "format=avif,quality=70,width=1000,height=1000,fit=contain";
  ...
  const imagePathAndNameWithOldExt = originalImageUrl.substring(STATIC_DOMAIN_PREFIX.length);
  const lastDotIndex = imagePathAndNameWithOldExt.lastIndexOf('.');
  const imageNameWithoutExtension = lastDotIndex === -1 ? imagePathAndNameWithOldExt : imagePathAndNameWithOldExt.substring(0, lastDotIndex);

  try {
    console.log(`Attempting to download transformed image: ${transformedImageUrl}`);
    const response = await fetch(transformedImageUrl);
    if (!response.ok) {
      throw new Error(`Failed to download transformed image: ${response.status} ${response.statusText}`);
    }
    imageBuffer = await response.arrayBuffer();
    newFileName = `${imageNameWithoutExtension}.avif`;
    finalImageUrlForS3 = transformedImageUrl;
    console.log(`Successfully downloaded transformed image. New file name: ${newFileName}`);
  } catch (transformError) {
    console.warn(`Failed to transform image ID: ${metaImage.id}. Error: ${(transformError as Error).message}`);
    console.log(`Falling back to original image: ${originalImageUrl}`);
    try {
      const response = await fetch(originalImageUrl);
      if (!response.ok) {
        throw new Error(`Failed to download original image: ${response.status} ${response.statusText}`);
      }
      imageBuffer = await response.arrayBuffer();
      ...
      console.log(`Successfully downloaded original image. New file name: ${newFileName}`);
    } catch (originalDownloadError) {
      console.error(`Failed to download original image ID: ${metaImage.id}. Error: ${(originalDownloadError as Error).message}`);
      return; // Skip this image
    }
  }

  if (!imageBuffer || !newFileName) {
    console.error(`Could not obtain image buffer or filename for image ID: ${metaImage.id}`);
    return;
  }
  ...
  try {
    ...
    console.log(`Successfully uploaded image ID: ${metaImage.id} to S3. New URL: ${uploadedImageUrl}`);

    await db.update(metaImages)
    ...
    console.error(`Error during S3 upload or DB update for image ID: ${metaImage.id}. Error: ${(uploadOrDbError as Error).message}`);
  }
}
