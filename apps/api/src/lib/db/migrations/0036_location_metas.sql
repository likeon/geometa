...
ALTER TABLE "map_group_location_metas" ADD CONSTRAINT "map_group_location_metas_location_id_map_group_locations_id_fk" FOREIGN KEY ("location_id") REFERENCES "public"."map_group_locations"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "map_group_location_metas" ADD CONSTRAINT "map_group_location_metas_meta_id_metas_id_fk" FOREIGN KEY ("meta_id") REFERENCES "public"."metas"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
FROM map_group_locations mgl
    ...
    JOIN metas m ON m.id = lm.meta_id;--> statement-breakpoint
...
SELECT m.id AS map_id,
 ...
 WHERE (mml.meta_id IS NOT NULL OR mwl.map_id IS NULL) AND (pi.map_id IS NOT NULL OR mwi.map_id IS NULL) AND pe.map_id IS NULL;--> statement-breakpoint
