...
"local")
  ...
  psql "${DATABASE_URL_POSTGRES_DB:-postgresql://postgres:postgres@localhost:5432/postgres}" -f "$SCRIPT_DIR/make-backup.sql" >>/dev/null
...
"prod" | "preview")
  ...
  echo "drop database if exists geometa with ( force ); create database geometa;" | psql "${DATABASE_URL_POSTGRES_DB:-postgresql://postgres:postgres@localhost:5432/postgres}"

  echo "Restoring dump to local database..."
  psql "${DATABASE_URL:-postgresql://postgres:postgres@localhost:5432/geometa}" -f "$SCRIPT_DIR/dump.sql"
