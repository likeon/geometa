...
class MoveCursorToPreviousUnfoldedLine {
  ...
  perform() {
    ...
    const lineNo = lines.findIndex((l) => {
      return cursor.ch === l.from.ch + list.getCheckboxLength() && cursor.line === l.from.line;
    });
    if (lineNo === 0) {
...
class ArrowLeftAndCtrlArrowLeftBehaviourOverride {
  constructor(plugin, settings, imeDetector, operationPerformer) {
    ...
    this.check = () => {
      return this.settings.keepCursorWithinContent !== 'never' && !this.imeDetector.isOpened();
    };
    this.run = (editor) => {
  ...
  mergeWithPreviousItem(root, cursor, list) {
    ...
    if (bothAreEmpty || prevIsEmptyAndSameLevel || listIsEmptyAndPrevIsParent) {
      ...
      if (!prev.getNotesIndent() && list.getNotesIndent()) {
        prev.setNotesIndent(
          prev.getFirstLineIndent() + list.getNotesIndent().slice(list.getFirstLineIndent().length)
        );
      }
...
class BetterListsStyles {
  constructor(settings, obsidianSettings) {
    ...
    this.updateBodyClass = () => {
      const shouldExists =
        this.obsidianSettings.isDefaultThemeEnabled() && this.settings.betterListsStyles;
...
class SelectAllContent {
  ...
  perform() {
    ...
    if (selectionFrom.line < rootStart.line || selectionTo.line > rootEnd.line) {
      return false;
    }
    if (
...
class DragAndDrop {
  constructor(plugin, settings, obisidian, parser, operationPerformer) {
    ...
    this.handleMouseDown = (e) => {
      if (!isFeatureSupported() || !this.settings.dragAndDrop || !isClickOnBullet(e)) {
        return;
      }
  ...
  load() {
    return __awaiter(this, void 0, void 0, function* () {
      this.plugin.registerEditorExtension([draggingLinesStateField, droppingLinesStateField]);
  ...
  drawDropZone() {
    ...
    const newParentIsRootList = !newParent.getParent();
    {
      const width = Math.round(
        view.contentDOM.offsetWidth - (dropVariant.left - this.state.leftPadding)
      );
...
class CreateNewItem {
  ...
  perform() {
    ...
    const bullet =
      onChildLevel && hasChildren ? list.getChildren()[0].getBullet() : list.getBullet();
...
function createEditorCallback(cb) {
  return (editor) => {
    ...
    if (!shouldStopPropagation && window.event && window.event.type === 'keydown') {
...
class ListsMovementCommands {
  constructor(plugin, obsidianSettings, operationPerformer) {
    ...
    this.indentList = (editor) => {
      const { shouldStopPropagation } = this.operationPerformer.perform(
        (root) => new IndentList(root, this.obsidianSettings.getDefaultIndentChars()),
        editor
      );
      return shouldStopPropagation;
    };
...
class ObsidianOutlinerPluginSettingTab extends obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle((toggle) => {
        toggle.setValue(this.settings.overrideTabBehaviour).onChange((value) =>
          __awaiter(this, void 0, void 0, function* () {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle((toggle) => {
        toggle.setValue(this.settings.overrideEnterBehaviour).onChange((value) =>
          __awaiter(this, void 0, void 0, function* () {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle((toggle) => {
        toggle.setValue(this.settings.overrideVimOBehaviour).onChange((value) =>
          __awaiter(this, void 0, void 0, function* () {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle((toggle) => {
        toggle.setValue(this.settings.overrideSelectAllBehaviour).onChange((value) =>
          __awaiter(this, void 0, void 0, function* () {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle((toggle) => {
        toggle.setValue(this.settings.betterListsStyles).onChange((value) =>
          __awaiter(this, void 0, void 0, function* () {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle((toggle) => {
        toggle.setValue(this.settings.verticalLines).onChange((value) =>
          __awaiter(this, void 0, void 0, function* () {
    ...
    new obsidian.Setting(containerEl).setName('Drag-and-Drop').addToggle((toggle) => {
      toggle.setValue(this.settings.dragAndDrop).onChange((value) =>
        __awaiter(this, void 0, void 0, function* () {
  ...
  load() {
    return __awaiter(this, void 0, void 0, function* () {
      this.plugin.addSettingTab(
        new ObsidianOutlinerPluginSettingTab(this.plugin.app, this.plugin, this.settings)
      );
    });
  }
  unload() {
...
class VerticalLinesPluginValue {
  ...
  recursive(list, parentCtx = {}) {
    ...
    const tillOffset = this.editor.posToOffset({
      line: nextSibling ? nextSibling.getFirstLineContentStart().line - 1 : this.lastLine,
      ch: 0
    });
    ...
    const top =
      visibleFrom > 0 && fromOffset < visibleFrom ? -20 : this.view.lineBlockAt(fromOffset).top;
    const bottom =
    ...
    if (height > 0 && !list.isFolded()) {
      ...
      const hasNextSibling =
        !!nextSibling &&
        this.editor.posToOffset(nextSibling.getFirstLineContentStart()) <= visibleTo;
      this.lines.push({
        top,
        left,
  ...
  updateDom() {
    ...
    this.contentContainer.style.marginLeft = cmContentContainer.offsetLeft + 'px';
    this.contentContainer.style.marginTop = cmContent.firstElementChild.offsetTop - 24 + 'px';
    for (let i = 0; i < this.lines.length; i++) {
...
class VerticalLines {
  constructor(plugin, settings, obsidianSettings, parser) {
    ...
    this.updateBodyClass = () => {
      const shouldExists =
        this.obsidianSettings.isDefaultThemeEnabled() && this.settings.verticalLines;
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      this.plugin.registerEditorExtension(
        view.ViewPlugin.define(
          (view) =>
            new VerticalLinesPluginValue(this.settings, this.obsidianSettings, this.parser, view)
        )
      );
    });
  }
    ...
    this.handleSettingsChange = () => {
      ...
      vim.defineAction('insertLineAfterBullet', (cm, operatorArgs) => {
        ...
        const res = operationPerformer.eval(
          root,
          new CreateNewItem(root, defaultIndentChars, getZoomRange, operatorArgs.after),
          editor
        );
        if (res.shouldUpdate && zoomRange) {
          editor.tryRefreshZoom(zoomRange.from.line);
