...
const coordinates = [{ panoId: 'pano-a', lat: 1, lng: 2, heading: 3, pitch: 4, zoom: 5 }];
...
describe('GeoGuessr draft coordinates', () => {
  ...
  test('prefers the current coordinates field and safely handles a missing field', () => {
    expect(getGeoguessrDraftCoordinates({ coordinates, customCoordinates: [] })).toBe(coordinates);
